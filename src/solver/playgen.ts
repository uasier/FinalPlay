...
import { Counts, Play, RankValue } from "./types";
...
export function generateAllPlays(counts: Counts): Play[] {
  const plays: Play[] = [];

  const sj = 13 as RankValue;
  const bj = 14 as RankValue;
  if (hasCount(counts, sj, 1) && hasCount(counts, bj, 1)) {
  ...
  for (let r = 0 as RankValue; r <= 14; r = ((r + 1) as RankValue)) {
    if (hasCount(counts, r, 1)) plays.push({ type: "single", keyRank: r, cards: [r] });
    if (hasCount(counts, r, 2)) plays.push({ type: "pair", keyRank: r, cards: [r, r] });
    if (hasCount(counts, r, 3)) plays.push({ type: "triple", keyRank: r, cards: [r, r, r] });
  }
  ...
  for (const run of enumerateRuns(tripleRunRanks)) {
    ...
    for (let start = run.start; start <= run.end; start = ((start + 1) as RankValue)) {
      for (let end = (start + 1) as RankValue; end <= run.end; end = ((end + 1) as RankValue)) {
        ...
        plays.push({ type: "airplane", keyRank: end, length: units, cards: baseCards });
